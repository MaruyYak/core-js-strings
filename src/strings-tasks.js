...
function removeFirstOccurrences(str, value) {
  ...
  return (
    str.slice(0, stringIndex) +
    str.slice(stringIndex + value.length, str.length)
  );
}
...
function isPalindrome(str) {
  ...
  return (
    noSign.toLowerCase() === noSign.toLowerCase().split('').reverse().join('')
  );
}
...
function findLongestWord(sentence) {
  const sepWords = sentence.split(/\s+/);
  let longWord = '';
  for (let i = 0; i < sepWords.length; i += 1) {
    if (sepWords[i].length > longWord.length) {
      longWord = sepWords[i];
    }
  }
  return longWord;
...
function encodeToRot13(str) {
  const alhb = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const cipher = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let cipherRes = '';
  for (let i = 0; i < str.length; i += 1) {
    const alhbIndex = alhb.indexOf(str[i]);
    if (alhbIndex !== -1) {
      cipherRes += cipher[alhbIndex];
    } else {
      cipherRes += str[i];
    }
  }
